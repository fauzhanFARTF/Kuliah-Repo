...
Public Class Form1
    ...
    Dim sql As String
    Public Sub koneksi()
        str = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=dtbpenjualan.mdb"
        conn = New OleDbConnection(str)
        If conn.State = ConnectionState.Closed Then conn.Open()
    End Sub
    Sub Tampilgrid()
        da = New OleDbDataAdapter("select * from tbpakaian", conn)
        ds = New DataSet
        da.Fill(ds, "tbpakaian")
        DataGridView1.DataSource = ds.Tables("tbpakaian")
    ...
    Sub save()
        ...
        Else
            Call koneksi()
            Try
                Dim simpan As String = "insert into tbpakaian (kodebaju, namabaju, ukuran, harga) values ('" & TextBox1.Text & "','" & ComboBox1.Text & "','" & ComboBox2.Text & "','" & TextBox2.Text & "')"
    ...
    Sub updated()
        If MessageBox.Show("Apakah Anda Ingin Megupdate ?", "SUKSES", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = vbYes Then
            sql = "update tbpakaian set namabaju = '" & ComboBox1.Text & "', ukuran = '" & ComboBox2.Text & "', harga ='" & TextBox2.Text & "' where kodebaju = '" & TextBox1.Text & "'"
    ...
    Sub deleted()
        ...
        Else
            If MessageBox.Show("Apakah Data Akan Di Hapus?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                Dim hapus As String = "Delete * from tbpakaian where kodebaju='" & TextBox1.Text & "'"
    ...
    Private Sub TextBox1_LostFocus(sender As Object, e As EventArgs) Handles TextBox1.LostFocus
        str = "SELECT * FROM tbpakaian where kodebaju ='" & TextBox1.Text & "'"
        cmd = New OleDbCommand(str, conn)
        rd = cmd.ExecuteReader
        Try
            While rd.Read
                ComboBox1.Text = rd.GetString(1)
                ComboBox2.Text = rd.GetValue(2)
